...
protected:
    /// Tag type used by subclasses to skip parent's constructor_validate_and_infer_types().
    struct SkipValidateTag {};
    /// Protected constructor that skips validation (for subclasses with different input layouts).
    MOECompressed(const OutputVector& args, const Config& config, SkipValidateTag);
