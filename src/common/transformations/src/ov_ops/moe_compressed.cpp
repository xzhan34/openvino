...
MOECompressed::MOECompressed(const OutputVector& args, const Config& config, SkipValidateTag)
    : MOE(args, config), m_config(config) {
    // Subclass will call constructor_validate_and_infer_types() itself,
    // dispatching to its own validate_and_infer_types() override.
}
