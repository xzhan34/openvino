...
void XmlSerializer::append_rt_info(pugi::xml_node& node, ov::RTMap& attributes) {
    ...
    for (auto& item : attributes) {
        if (item.second.is<ov::RuntimeAttribute>()) {
            ...
            if (!m_deterministic || rt_attribute.is_deterministic()) {
...
void XmlSerializer::on_adapter(const std::string& name, ov::ValueAccessor<void>& adapter) {
    ...
    if (is_body_target) {
        ...
        // Bodies can be without parameters(depending on constants), but can not be without results
        OPENVINO_ASSERT(!result_mapping.empty(), "No results found in body Model.");
        // TI, Loop do not have attributes as regular ops, it is necessary to append "port_map" and
        // "back_edges" to layer above (m_xml_node.parent()) as in serialize() layer (here "m_xml_node")
...
void XmlSerializer::on_adapter(const std::string& name, ov::ValueAccessor<std::shared_ptr<ov::Model>>& adapter) {
    if (name.find("body") != std::string::npos) {
        ...
        // TI, Loop do not have attributes as regular ops, it is necessary to append "body"
        // to layer above (m_xml_node.parent()) as in serialize() layer (m_xml_node) with empty attributes
        // is removed.
